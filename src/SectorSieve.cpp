...
SectorSieve::SectorSieve(uint64_t x, long double alpha, long double beta, bool verbose)
    ...
    : SieveTemplate<bool>(x, verbose), x(x) // norm bound
      ,
...
{
  if (beta < alpha)
  { // the order of alpha and beta doesn't matter; corrected here
...
void SectorSieve::crossOffMultiples(gint g)
{
  ...
  { //ignoring c, d = 0, 0
    ...
    {
      try
      {                                                                       // round-off error may make this go out of bounds
